...
chrome.storage.sync.get(['filterEnabled', 'animatedGradientEnabled', 'simpleProgressBarEnabled'], function(data) {
  ...
  if (data.filterEnabled === false) {
    ...
    if (data.animatedGradientEnabled && !data.simpleProgressBarEnabled) {
      progressText.textContent = `${getRandomPokemonName()} - AI 过滤准备中... (0 / ${totalTopics})`;
    } else {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "updateProgress") {
      ...
      if (data.animatedGradientEnabled && !data.simpleProgressBarEnabled) {
        statusText = `${getRandomPokemonName()} - AI 过滤中: ${processedTopics} / ${totalTopics} (${progress.toFixed(0)}%)`;
      } else {
        // 计算并显示剩余时间
